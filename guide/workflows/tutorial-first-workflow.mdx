...
<Step title="First Step - Creating a new Workflow">
  ...
  At this point you shoud be in the workflow <u>sandbox</u> and the workflow has already been created.
...
<Step title="Third Step - Configuring the OpenAI Asistant Node">
  ...
  - **Model** is the AI brain that your users will be talking to, there are more powerfull and expensive models as well as simpler, cheap and blazing fast models. You can choose any model you like, in this tutorial we will be using `GPT-4o`.
  ...
  - **Max output tokens** will depend on the model you chose, if you choose a powerfull model, you should bump this setting up. Otherwise, you can take it a bit slower. In this tutorial, we will use a maximum of `400` tokens.
    > Tokens are the base element for AI processes. Its how the AI segments your text before generating a response.
...
<Step title="Fourth Step - Configuring the WhatsApp Text Message Node">
  ...
  - **Phone Number WhatsApp ID** is the WhatsApp ID of the phone number that will send that message. This field will list all your WhatsApp phone numbers connected to Cogfy.
    You can leave it blank or choose *Use fallback value* to use the phone number that received the message in the trigger node, which is our case in this tutorial.
    <Info>
  ...
  <img
    src="/guide/workflows/images/tutorial-first-workflow/complete-whatsapp-text-message-node.png"
...
<Step title="Sixth Step - Publishing Your Workflow">
  ...
  Select your wokflow and hit *Save*.
